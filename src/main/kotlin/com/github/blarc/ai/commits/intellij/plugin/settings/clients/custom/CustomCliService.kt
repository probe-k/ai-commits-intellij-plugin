...
class CustomCliService(private val cs: CoroutineScope) : LLMClientService<CustomCliConfiguration>(cs) {
    ...
    override suspend fun makeRequest(client: CustomCliConfiguration, text: String, onSuccess: suspend (r: String) -> Unit, onError: suspend (r: String) -> Unit) {
        makeRequestWithTryCatch(function = {
            ...
            try {
                ...
                if (process.exitValue() != 0) {
                    throw RuntimeException("${client.command} CLI 실패 (exit code: ${process.exitValue()}): $output")
                }

                onSuccess(output.toString().trim())
            } catch (e: TimeoutCancellationException) {
                process.destroyForcibly() // 프로세스 강제 종료
                throw RuntimeException("${client.command} CLI 호출이 제한 시간(${client.timeout}초) 안에 끝나지 않아 강제 종료되었습니다.")
            }
        }, onError = onError)
    }
}
